...
- Read-only implemented endpoints are available without mutation auth on the local daemon.
...
| ----------------------------------------------- | ------------ | ----------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/__hub/api/state`                              | `GET`        | Implemented | Read-only                       | `{ "apps": AppState[], "groups": AppGroup[], "components": AppComponent[], "diagnostics"?: Diagnostic[], "generatedAt": string }`                                                                                                                                                                    |
| `/__hub/api/apps`                               | `GET`        | Implemented | Read-only                       | `{ "apps": AppStatus[] }`                                                                                                                                                                                                                                                                            |
| `/__hub/api/apps/:id`                           | `GET`        | Planned     | Read-only                       | Not implemented. Current app detail endpoint is `/__hub/api/apps/:id/state`.                                                                                                                                                                                                                         |
| `/__hub/api/apps/:id/state`                     | `GET`        | Implemented | Read-only                       | `{ "state": AppState }`                                                                                                                                                                                                                                                                              |
...
| `/__hub/api/apps/:id/logs`                      | `GET`        | Implemented | Read-only                       | Durable snapshot with paging: `{ "id": string, "logs": string[], "events": LogEvent[], "page": LogPage, "diagnostics": Diagnostic[], "streamUrl": string }`                                                                                                                                          |
| `/__hub/api/apps/:id/logs/stream`               | `GET`        | Implemented | Read-only                       | SSE events: `status`, `snapshot`, `log`, `ping`.                                                                                                                                                                                                                                                     |
...
| `/__hub/api/agent/sessions/:id/messages`        | `POST`       | Implemented | Token                           | Accepts `{ "content": string, "context"?: TuiAgentContext }`; appends a redacted user message. Disabled or missing config returns a diagnostic failed run. Enabled, configured runs invoke the daemon Operator Agent runtime and may return streamed model output plus a redacted assistant message. |
| `/__hub/api/agent/sessions/:id/events`          | `GET`        | Implemented | Token                           | SSE stream of typed `AgentRunEvent` payloads. `afterSequence=<n>` and `Last-Event-ID` replay stored events newer than the requested sequence while still reporting `snapshot_only` recovery metadata; clients should refresh the session snapshot after reconnect.                                   |
...
- read-only: `list_apps`, `get_app_state`, `get_app_group`, `get_diagnostics`, `tail_logs`, `search_logs`, `detect_project`, `plan_app_setup`, `preview_setup_writes`, `inspect_manifest`, `validate_manifest`, `repair_app_setup`, `propose_tui_action`
...
`OperationStore` is in-memory today. R004 bridges its internal `subscribe` hook into the global `/__hub/api/events` stream.
