...
- Public state/list/operation read endpoints are available without mutation auth on the local daemon. Log reads, event streams, Agent Gateway routes, export routes, and mutation routes require the local Relaybase token because they can expose sensitive operational data or perform actions.
