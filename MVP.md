...
- 2026-05-12 01:23:52 -05:00 - Reorganized README and documentation into a concise README plus focused architecture, CLI, manifest, MCP, app-state, security/limits, and development guides, then verified formatting, tests, smoke, and package contents.
- 2026-05-12 14:19:31 -05:00 - Added a Windows-safe `relaybase-dev.cmd` helper wrapper for Codex App, updated helper docs to avoid direct `.ps1` invocation, and added static plus live Windows smoke coverage for process-local execution-policy bypass.
- 2026-05-12 14:25:06 -05:00 - Minimized the Windows helper wrapper documentation so the `.cmd` guidance stays visible without repeating policy details across the docs.
- 2026-05-12 15:39:14 -05:00 - Implemented production-grade Docker Compose setup hardening with explicit service/port/health/timing inputs, conservative ambiguous-service blocking, dependency port reset overrides, streaming Compose hooks, Windows Docker Desktop recovery approval, `health --prove` proof bundles, MCP `prove_app`, helper `docker-prove`, docs, and edge-case tests.
- 2026-05-12 15:44:24 -05:00 - Minimally polished Docker and proof documentation for factual wording around conservative service selection, profile stop budgets, dependency port policy, Docker Desktop recovery approval, and helper proof diagnostics.
